...
int main(int argc, char *args[])
{
    ...
    // Use named semaphore
    mutex_sem = sem_open("/mutex_sem", O_CREAT, 0644, 1);
    if (mutex_sem == SEM_FAILED) {
        perror("Failed to open semphore for mutex_sem");
        exit(EXIT_FAILURE);
    }

    data_available_sem = sem_open("/data_available_sem", O_CREAT, 0644, 0);
    if (data_available_sem == SEM_FAILED) {
        perror("Failed to open semphore for data_available_sem");
        exit(EXIT_FAILURE);
    }

    room_vailable_sem = sem_open("/room_vailable_sem", O_CREAT, 0644, BUFFER_SIZE);
